...
3. **抽取結果要先驗收，才能當來源用。** 前兩條管的是「有沒有讀原文」，這條管的是「讀到的是不是完整的原文」。動態載入的頁面會讓同一個 URL 在不同時刻回傳不同內容，而兩次都長得像官方頁面。驗收清單見下。

### 抽取結果的驗收（判 Confirmed 之前跑一次）

| 症狀 | 意義 | 動作 |
|---|---|---|
| 內容裡出現 `Loading`、`...`、空的 list item | 動態欄位還沒填 | **視為抽取失敗**，重抓；不可拿這份內容判定任何 verdict |
| 該有的欄位整個不見（價格、語言、版本、日期） | 同上，常見於 SPA 型的官方文件站 | 重抓；仍缺就換工具（`firecrawl_scrape` 加 `waitFor`）或改用瀏覽器 |
| 同一 URL 兩次抽取結果不同 | **先懷疑自己的抽取，再懷疑對方的頁面** | 以欄位較完整的那份為準；不要寫成「官方頁面前後不一致」 |
| 兩個不同 URL 但 title 完全相同 | 很可能是同一頁（其一為 redirect / alias） | 比對 canonical 或內文，別把它當成「兩個頁面互相矛盾」 |

**為什麼要特別列這條**：本站踩過一次——把 Microsoft AI-103 的語言清單寫成「認證總覽頁與考試頁不一致」，還當作「同網域不同頁新鮮度會差」的例證。實際上那是同一頁的價格與語言欄位動態載入，抓取工具在載入完成前取到半成品；同一份抽取裡「Skills earned upon completion」還顯示 `Loading`，線索就在眼前但沒被當回事。**用抓取偽影去佐證一條真的規則，比單純抄錯數字更難發現**，因為結論聽起來很合理。
...
| 「抓回來就是官方內容，直接拿來判定」 | 動態載入的頁面會回半成品，而半成品也長得像官方頁。判 Confirmed 前先跑抽取驗收清單 |
| 「同一 URL 兩次結果不同 → 官方頁面在改版」 | 先懷疑自己的抽取。真的是頁面變動的話，兩份都要有完整欄位才算數 |
