...
```bash
grep -nP "(?<!產)品類(?!別)|貼標|機構記憶|視頻|網絡|信號|界面|用戶|質量|默認|博客(?!來)|審計|(?<!確)保安(?!全)|賦能|抓手|(?<!封)閉環|對標|復盤|顆粒度" src/content/posts/<category>/<file>.md
```

## 長文的體裁與語域（1500 字以上必讀）

長文最常見的失敗不是寫錯，是**讀者讀不下去**。以下六條在下筆前就決定，事後補救要重寫。
...
六項指標與閾值見 `post-polish` skill。機器抓不到第 1、2、3 條，那三條要自己讀。
