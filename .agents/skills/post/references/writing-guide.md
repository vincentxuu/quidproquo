...
**tech / ai（專文介紹型 / `type: deep-dive`）**：適合介紹工具、技術堆疊、架構設計。目標長度 1000-2000 字。不需要大量第一人稱經驗，介紹文的價值在於讓讀者理解工具、做出選擇。每個段落至少展開：設計哲學 / 跟常見替代方案的比較 / 適合與不適合的情境 / 具體用法 / 限制。有程式碼範例就加，流程/架構圖優先用 mermaid（見下方§圖解），畫不出來或太複雜再退回 ASCII。標題直接點主題，不要加「的解法」。
...
長文讀不下去，常常不是句子的問題，是**該畫圖的地方用文字硬撐**。多步驟流程、有分支的決策邏輯、多個概念的比較、時間軸——這些讀者要在腦中組一張圖才看得懂，直接給圖比逼讀者自己組快很多。

站上參考的範例是 [BigGo 財經的 podcast 逐字稿導讀](https://finance.biggo.com.tw/podcast/9df860791be6c046)：長文中每隔幾段就插一張圖，把「模式 A vs 模式 B」「舊流程 vs 新流程」這種需要對照著看的內容畫出來，而不是全部塞進段落裡讓讀者自己比對。

### 何時要畫

- **有分支的流程**：「如果 A 就走這條、如果 B 就走那條」——文字寫超過兩層 if/else 就該畫
- **多步驟的因果鏈**：三步以上、每步都影響下一步（例如 `stdin → 解析 → 分派 → 執行`）
- **並排比較**：新舊模式、兩種架構、幾個方案的取捨——尤其是本指南前面「§正文每個主張最多一個數字」建議收進附錄的細節，畫成圖比列成表更快抓到差異
- **時間軸 / 演進**：一個技術或流程如何一步步變成現在的樣子

### 何時不要畫

- 單一、線性的概念（A 導致 B，沒有分支）——一句話講得完就不要畫
- 只是想讓文章看起來更豐富——沒有資訊密度的裝飾圖，讀者會跳過
- 內容其實是一張表（分類、對照、規格比較）——用 Markdown 表格，不要硬畫成圖
- 節點數會超過 10-12 個——太複雜的圖在文章寬度（含手機）下擠成一團，讀者一樣讀不懂，這種情況拆成兩張圖或改用文字分點

### 怎麼寫

用 fenced code block，語言標 `mermaid`：
...
本站已接上真正的渲染（client-side，載入後把這個區塊換成實際圖表，非渲染環境會 fallback 成原始語法區塊），寫的時候注意：

- 節點文字要短，中文一個節點盡量壓在 8-10 字內，太長的敘述搬到圖外的段落說明
- 決策分支的標籤（`-- 是 -->`）要講清楚條件，不要只寫「是/否」以外還要看上下文才懂的字
- `flowchart TD`（直向）通常比 `flowchart LR`（橫向）更適合手機寬度，橫向圖節點一多就要橫向捲動
...
- [ ] 有多步驟流程、分支決策、或需要並排比較的內容嗎——有的話規劃畫成 mermaid 圖（§圖解），不要全部塞成文字段落
