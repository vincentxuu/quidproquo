...
台灣本來就在用、不用改：`資料`、`數據`、`優化`、`部署`、`落地`、`視角`、`通過認證`、`文件`（指 document）。
...
```bash
grep -nE "品類|貼標|機構記憶|補丁|視頻|網絡|信號|界面|用戶|質量|項目|默認|博客|審計|保安|賦能|抓手|閉環|對標|復盤|顆粒度" src/content/posts/<category>/<file>.md
```
