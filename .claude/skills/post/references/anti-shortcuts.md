...
| 「課程 / 工具 / 站上文章提名字就好，連結以後再補」 | 每個第一次出現的專名——課號（CS221、10-301）、課程官網、工具、同系列的站上文章——當下就要是超連結。「之後再補」不會發生，本站踩過：course map 總覽初稿把四間學校地圖與多門課全裸寫成純文字，上線後讀者只能自己搜尋。commit 前逐段掃：看到專名但不是 `[文字](url)` → 補上 |
