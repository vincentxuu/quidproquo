...
```bash
pnpm check:tw-usage                      # 預設只掃你改動過的文章
pnpm check:tw-usage <path>               # 指定單篇
pnpm audit:tw-usage                      # 全站稽核，只報告不擋
```

`pnpm verify` 會跑第一種形式，所以**新增或修改的 zh-TW 文章沒清乾淨就 commit 不了**。既有語料寫在這道檢查之前，全站還有一百多處待清，那是稽核模式的範圍，不擋 commit。

詞表維護在 `scripts/check-tw-usage.mjs`，分兩層：`BLOCKED`（硬擋）與 `REVIEW`（只報告）。要新增詞就改那裡，不要只改這份文件——**文件裡的詞表如果沒有進腳本，等於沒人會跑**。

真正的例外（物理的「質量」、貨物「卸載」）在該行前一行加 `<!-- tw-usage-ignore -->`；整篇不適用就放 `<!-- tw-usage-ignore-file -->`。
...
|---|---|---|
| 類別 | 品類 | 中國行銷術語 |
| 貼上 X 的標籤 | 貼標成 X | 「貼標籤」本身沒問題 |
| 組織記憶 | 機構記憶 | institutional memory；台灣管理學譯法 |
| 影片 / 網路 / 訊號 / 資訊 / 介面 | 視頻 / 網絡 / 信號 / 信息 / 界面 | |
| 使用者 / 品質 / 預設 | 用戶 / 質量 / 默認、缺省 | |
| 部落格 / 稽核 / 保全 | 博客 / 審計 / 保安 | |
| 說明 / 做法 / 成效 | 賦能 / 抓手 / 閉環 / 對標 / 復盤 / 顆粒度 | 中國商業黑話，一律不用 |
| 規模 / 數量 | 量級 | |
| 工作流程 / workflow | 工作流 | |
| 呼叫 / 啟用 / 列印 / 連接埠 | 調用 / 激活 / 打印 / 端口 | |
| 軟體 / 硬體 / 記憶體 / 硬碟 / 伺服器 / 螢幕 / 信箱 | 軟件 / 硬件 / 內存 / 硬盤 / 服務器 / 屏幕 / 郵箱 | |
| 小黃鴨除錯法 | 橡皮鴨 | rubber duck debugging；台灣「橡皮」是 eraser |
| 馬上 / 可靠 | 立馬 / 靠譜 | |
...
| 詞 | 什麼時候要改 |
|---|---|
| `項目` | 指 project 要改「專案」；品項（檢查項目、保障項目）可留 |
| `天花板` | ceiling 直譯要改「上限」；台灣自有的比喻用法可留 |
| `鷹架` | 教育學的 scaffolding 可留；把 scaffolding 硬譯成鷹架要改 |
| `阻塞` | 網路 / OS 教科書用法可留；blocking 直譯建議改「卡住」 |
| `對齊` | align 直譯，多半可改「講清楚方向」「照著走」 |
| `卸載` | 軟體要改「解除安裝」；貨物卸載可留 |

台灣本來就在用、不用改：`資料`、`數據`、`優化`、`部署`、`落地`、`視角`、`迭代`、`標籤`、`通過認證`、`文件`（指 document）、`補丁`（資安補丁、更新補丁）。
...
**腳本抓不到這一段，只能逐句看。** 英文材料寫成的稿子最容易留下這五種痕跡：

1. **形容詞直譯**：「很不性感」（unglamorous）→「很不起眼」
2. **名詞化**：「國家層級的執法啟動」→「國家層級開始執法」
3. **英文語序**：「辯護每個選擇」（defend each choice）→「為每個選擇辯護」
4. **術語硬譯**：framing →「框架」在中文常不通，多半該寫「說法」「詮釋」「講法」
5. **隱喻直譯**：ceiling →「天花板」、scaffolding →「鷹架」、ticket →「票」、blocking →「阻塞」。英文的比喻在中文常常不是同一個比喻，照字面搬會生出沒人這樣講的詞

### 命中要列表回報，不要靜默修掉

掃完（詞彙 + 翻譯腔）**先把清單交給使用者**，一行一個：原文 → 問題 → 建議改法。理由有三個：

- 有些命中是刻意的（引述原文、專有名詞、物理的「質量」），靜默改掉就是改錯
- 使用者看得到清單才知道這篇被動了哪些地方，不必自己 diff
- 詞彙判斷本來就有主觀成分，列出來使用者可以只採納一部分

改完回報「修了幾處、保留幾處、保留的理由」。**不要只寫「已修正台灣用語」**——那句話沒有任何資訊。

### 其他慣例

- 金額用「4,000 萬美元」，不要「$4,000 萬」——幣別跟在單位後面
- 法條、報告名稱第一次出現給原文，不要自創中譯後就再也不附原文
- 引用還在密集改動的來源（活躍維護的 repo、會改版的官方文件）時，標出**對照的版本或日期**。否則三個月後讀者無從判斷文章寫的是不是現況
...
寫後：
- [ ] frontmatter 必填欄位齊全
- [ ] `lang: zh-TW` 的文章 `pnpm check:tw-usage` 全綠，「人工判讀」那組每一筆都看過
- [ ] 翻譯腔五型逐句看過，命中清單已列給使用者（不是靜默修掉）
