...
Production wiring is generated by `scripts/create-cron-entry.mjs` after `astro build`. It writes `dist/cron-entry.js`, imports Astro's generated `./server/entry.mjs`, forwards `queue()` to `handleQueueBatch`, and patches `dist/server/wrangler.json` so Wrangler bundles the TypeScript imports. Keep durable queue logic in `src/server/queue.ts`; generated `dist/` output is deploy glue only.

## Cron Orchestrator Wiring

Cloudflare Cron Triggers invoke the Worker's `scheduled()` handler, not arbitrary HTTP endpoints. The generated `dist/cron-entry.js` compares `event.cron` against `scheduledAgentEntries`, calls `checkKernelHealth(env)` on `0 3 * * *`, and POSTs each due agent to `/api/admin/agents/scheduled` with `X-Crawl-Secret`.
