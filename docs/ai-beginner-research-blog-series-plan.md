...
- **每篇只服務一種人、只做一件事**。分眾規則見下一節——寫的時候要能一句話說出「這篇是寫給誰、要他做什麼」，說不出來就是題目沒切乾淨。

---

## 3. 讀者分眾

「AI 小白」是系列的**入口**，不是全部的讀者。11 篇實際上服務四種人，而且他們是**不同時間點的同一個人**——所以分眾同時是分眾，也是路線圖。

### 四種讀者

| 代號 | 是誰 | 現在卡在哪 | 他要的是 |
|---|---|---|---|
| **A 觀望者** | 沒開過終端機，聽說 AI 可以幫忙寫東西 | 不知道自己能不能做、值不值得做 | 一個能說服自己開始的理由，和一個不會卡死的第一步 |
| **B 手動派** | 會用 ChatGPT / Claude 網頁版，寫過幾篇，覺得產出很空 | 產出品質，不是工具 | 讓 AI 寫的東西有來源、有立場、能被查證的方法 |
| **C 工具人** | 已經在用 Claude Code 或 Codex，臨場下指令 | 每次都要重講一遍，換工具就重來 | 把流程固化下來、而且可攜 |
| **D 營運者** | 已經跑了幾個月，開始出現熵 | 內容過期、tag 亂長、成本失控 | 治理機制與長期維護 |

### 五種目的（決定文章怎麼寫）

| 目的 | 成功長什麼樣 | 寫法 |
|---|---|---|
| **說服** | 讀者改變做法 | 有立場、有反面案例，不列功能 |
| **動手** | 讀者做出東西 | 逐步、可複製、有卡關排除 |
| **判準** | 讀者做出選擇 | 給判斷準則，不給推薦清單 |
| **參考** | 讀者之後回來查 | 表格化、可跳讀、標題可錨定 |
| **立場** | 讀者引用或反駁 | 一個明確主張，證據撐住 |

### 分眾 × 目的對照

| # | 主要對象 | 次要 | 目的 | 這篇成功的定義 |
|---|---|---|---|---|
| 1 | A | B | 說服 + 判準 | 讀者選定自己要用哪個介面，並願意裝 |
| 2 | A | — | 動手 | CLI 跑起來，且知道自己一個月要付多少 |
| 3 | A | — | 動手 + 判準 | 有一個能公開的網址 |
| 4 | A | C | 動手 | 不用再重講規則；兩邊都吃同一份 |
| 5 | **B** | C | 說服 + 動手 | 產出第一份有來源清單的 research note |
| 6 | **B** | A | 動手 | 發出第一篇有引用的文章 |
| 7 | **B** | D | **立場** | 讀者建立「不查證不發」的紀律 |
| 8 | **C** | B | 動手 + 參考 | 手上有一個自己寫的 skill |
| 9 | **C** | — | 參考 | 雙軌設定不再互相打架 |
| 10 | **D** | C | 立場 + 參考 | 建立一套自己的行動分級 |
| 11 | **D** | B | 動手 + 參考 | 文章被搜尋與 AI 引用到 |

### 三條入口動線

系列不是只有「從第 1 篇開始」一條路。搜尋進來的人會落在中段，所以**每篇開頭都要有一句「你如果是 X，可以從這裡開始」**：

- **A 動線（從零）**：1 → 2 → 3 → 4 → 5 → 6 → 7　（做出一個會運作的部落格）
- **B 動線（已經在寫，想寫好）**：**5 → 6 → 7** → 8　（可完全跳過建置三篇）
- **C/D 動線（已經在用 agent）**：**8 → 9 → 10** → 11　（把既有流程系統化）

三條動線各自有一篇「錨點文」——A 是第 3 篇（做出網址的成就感）、B 是第 7 篇（最有立場、最可能被引用）、C/D 是第 9 篇（最不可取代的技術內容）。這三篇值得投入最多篇幅與第一手素材。

### 分眾對寫法的硬性約束

- **第 5–7 篇不能假設讀者跑過第 1–4 篇**。B 動線的人可能用 Notion、用 WordPress、甚至只用網頁版 Claude——這三篇的方法必須跟平台脫鉤，工具只當示範。
- **第 8–11 篇可以假設讀者有終端機**，不用再解釋什麼是資料夾。這裡如果還在照顧 A，C 和 D 會棄讀。
- **第 1–4 篇不准出現 skill、MCP、progressive disclosure 這類詞**。A 在這階段每多一個新名詞就多一次流失。
- **每篇的第一段就要暴露對象**。不要讓 D 讀了三段才發現這篇是給小白的。
...
| # | 暫定標題 | 對象 | 讀完後手上有什麼 | type |
|---|---|---|---|---|
| 1 | Agent 不是 chatbot：研究型部落格的三段式產線 | A | 一張工具地圖 + 判斷該用哪個介面 | guide |
| 2 | 裝起來：Claude Code 與 Codex 的雙軌入門與費用真相 | A | 兩個能跑的 CLI、一個乾淨的專案資料夾 | guide |
| 3 | 讓 agent 幫你把部落格生出來並上線 | A | 一個有網址的部落格 | guide |
| 4 | AGENTS.md 與 CLAUDE.md：把「我是誰、我要什麼」寫下來 | A / C | 一份兩邊都吃的專案記憶 | guide |
| 5 | 研究不是叫 AI 搜尋：子問題、來源分級、交叉驗證 | B | 一份帶來源清單與事實交叉表的 research note | guide |
| 6 | 從 research note 到一篇文章 | B | 你的第一篇研究型文章 | guide |
| 7 | 發文前的三道關卡：審稿、查證、連結檢查 | B | 一份可重複執行的發文檢查清單 | guide |
| 8 | 把整條流程封裝成 skill，Claude 與 Codex 共用一份 | C | 你自己的 `post` / `research` skill | guide |

### 延伸 3 篇（核心跑順之後再出）

| # | 暫定標題 | 對象 | 主軸 | type |
|---|---|---|---|---|
| 9 | 雙軌兼容的工程細節 | C | skill 路徑差異、sync vs symlink、`/` vs `$`、context budget | deep-dive |
| 10 | 不要讓它長歪：治理、成本與內容過期 | D | 行動分級、verify gate、usage limit、freshness | deep-dive |
| 11 | 讓文章被找到：英文版、排程發布、AEO/GEO | D | 翻譯 pipeline、未來日期發布、結構化資料 | guide |
...
**對象**：A（次要 B）｜**目的**：說服 + 判準｜**成功**：讀者選定介面並願意裝
...
**對象**：A｜**目的**：動手｜**成功**：CLI 跑起來，且知道自己一個月要付多少
...
**對象**：A｜**目的**：動手 + 判準｜**成功**：有一個能公開的網址（A 動線的錨點文）
...
**對象**：A（次要 C）｜**目的**：動手｜**成功**：不用再重講規則，且兩邊都吃同一份

這是核心 8 篇裡唯一同時服務 A 和 C 的一篇：A 只需要「寫下來就不用再講」，C 需要「兩個工具吃同一份」。寫法上讓 A 讀完前半就能收工，相容性細節放後半並標示可跳過。
...
**對象**：B（次要 C）｜**目的**：說服 + 動手｜**成功**：產出第一份有來源清單的 research note

**B 動線的起點**。這篇要能被單獨搜到、單獨讀完，**不能預設讀者跑過第 1–4 篇**——方法必須跟平台脫鉤，用 Notion 或 WordPress 的人也要能照做。
...
**對象**：B（次要 A）｜**目的**：動手｜**成功**：發出第一篇有引用的文章
...
**對象**：B（次要 D）｜**目的**：**立場**｜**成功**：讀者建立「不查證不發」的紀律

**B 動線的錨點文，也是全系列最可能被引用與轉載的一篇**。目的不是教步驟，是讓人改變做法——所以要有明確主張、有證據，篇幅可以最長。
...
**對象**：C（次要 B）｜**目的**：動手 + 參考｜**成功**：手上有一個自己寫的 skill

**系列的分水嶺**。這篇之後可以假設讀者有終端機、看得懂路徑，不再解釋基礎名詞——繼續照顧 A 會讓 C 和 D 棄讀。
...
**對象**：C｜**目的**：參考｜**成功**：雙軌設定不再互相打架

deep-dive，也是 **C/D 動線的錨點文**——最不可取代的技術內容，中文目前查不到。寫成可跳讀、可回來查的參考格式（表格為主），不用鋪陳。把第 8 篇刻意簡化掉的東西攤開。
...
**對象**：D（次要 C）｜**目的**：立場 + 參考｜**成功**：讀者建立一套自己的行動分級
...
**對象**：D（次要 B）｜**目的**：動手 + 參考｜**成功**：文章被搜尋與 AI 引用到
