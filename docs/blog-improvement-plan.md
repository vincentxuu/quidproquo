...
| Agent 進度追蹤 | ❌ 沒有 progress.txt | 文章最推崇的機制，自己卻沒用 |

> 「claude-progress.txt 是整個架構中最優雅的部分... 最低成本的 episodic memory 實作——不需要 vector database，一個文字檔就夠了。」但專案沒有任何等效機制，每次新 session 的 agent 都要重新理解整個專案。
...
| 項目 | 現況 | 行動 |
|------|------|------|
| Pre-commit hook | ❌ 完全沒有 | 加入 lint + reference check |
| `check-post-references.mjs` | ⚠️ 存在但不在 CI | 加入 CI pipeline |
...
專案現況：❌ 沒有任何 session-start hook。沒有自動環境驗證、沒有自動讀取進度、沒有 smoke test。Agent 可能在壞掉的環境中開始工作。
...
內容生產流程：❌ Post skill 沒有 Evaluator。Agent 寫完文章 → 直接 commit，沒有品質評估。
...
可做成 Claude Code Hook，在 git commit 前自動執行（類似 Stripe Minions 的 Blueprint 模式）。
...
| # | 項目 | 理由 |
|---|------|------|
| H1 | 建立根目錄 CLAUDE.md | 「Repository as Truth」原則。專案概覽、技術棧、開發流程、命名規範 |
| H2 | 建立 progress.txt | 最推崇的機制。記錄目前狀態、已完成項目、下一步 |
| H3 | 加 pre-commit hook（lint + reference check） | 「Linter 是法律」原則。不讓斷連結和 lint 錯誤進入 repo |
...
| 已就緒 | 尚未實作 |
|--------|----------|
| Crawl Pipeline（Browser Rendering → D1） | LangGraph Agent 節點 |
...
| 完整 RAG 設計文件 | Embedding Pipeline |
...
可做成 Claude Code Hook，在 git commit 前自動執行。
...
| # | 問題 | 來源 | 修正 |
|---|------|------|------|
| 9 | 缺少 Deterministic Validation 節點 | Stripe Blueprint 模式 | Writer → Critic 間插入確定性驗證（Markdown 語法、URL 存在性、回覆長度） |
| 10 | Critic 失敗無降級策略 | Stripe 2-attempt cap | 重試 2 次仍不通過 → 標註可能不完整 + 記錄 Langfuse + 不再呼叫 LLM |
...
| 漸進式摘要 | Section 5.5 Progressive Summarization |
