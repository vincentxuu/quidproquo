...
test('loads the canonical 21-case golden dataset with its Promptfoo retrieval contracts', () => {
  ...
  assert.deepEqual(
    promptfooTests.map((testCase) => testCase.vars.caseId),
    ['q05', 'q06', 'q09', 'q11', 'q15', 'q20', 'q21'],
  )
  assert.deepEqual(promptfooTests.at(-1).assert[0].config.requiredSources, [
...
test('loads the shared offline fixture without treating it as live evidence', () => {
  ...
  // Every case that carries a retrieval contract must have offline evidence,
  // otherwise `eval:promptfoo:fixture` errors with "No Promptfoo fixture found".
  const fixtureIds = new Set(fixture.cases.map((testCase) => testCase.id))
  for (const promptfooTest of buildPromptfooTests()) {
    assert.ok(fixtureIds.has(promptfooTest.vars.caseId), `missing fixture for ${promptfooTest.vars.caseId}`)
  }
})
