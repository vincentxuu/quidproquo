...
- [x] 4.2.2 Astro's Cloudflare adapter exposes the default Worker via the build output but does not natively forward `queue()` handlers — create `src/server/queue.ts` exporting `async function handleQueueBatch(batch, env, ctx)` and document in `src/server/queue.ts` header that this is invoked from a thin shim Worker entry registered in `astro.config.mjs` under `adapter.imageService` siblings via the `runtime.bindings` + `entry: 'src/server/worker.ts'` override (record this architectural choice in `docs/agent-os-runbook.md` since the adapter has no convention)
