...
| 精確匹配 | ✅ | ✅ | 弱 |
| 索引大小 | 小 | 小～中 | 大 |
| 搜尋速度 | 快 | 視實作而定（原版慢於 BM25，效率化變體才追平） | 慢（ANN） |
| 多語言 | 需要分詞器 | 依訓練資料 | ✅ |
...
SPLADE 是 BM25 的有力進化，但不是每個系統都需要它。Dense + BM25 + Multi-Query 的組合在很多場景已經夠好。SPLADE 的價值在於它補了一個特定的空缺：術語近義詞和縮寫擴展，而且用稀疏向量實作。速度要留意：[SPLADE 效率化那篇論文](https://arxiv.org/abs/2207.03834)的出發點就是「縮小 SPLADE 與傳統檢索的延遲差距」，能追平 BM25 的是它提出的效率化變體，不是原始 checkpoint。
