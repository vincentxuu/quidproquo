...
```
- 用國中生能理解的語言解釋技術概念
- 當問題超出 PostgreSQL 範圍時，說明這不在你的專業範圍內，並建議使用者諮詢相關專家
...
如果使用者問的問題你不確定答案就要說明你不確定，另外每次回答
都要先確認你理解了使用者的問題再開始回答...
...
```
使用者：PostgreSQL 17 的新 VACUUM 改進有哪些？
...
靜態 few-shot 的問題是：不管使用者問什麼，都看到同一組範例。

**動態 few-shot** 的做法是：根據使用者的輸入，從範例庫中檢索最相似的範例。

```
流程：

使用者輸入 → embedding → 相似度搜尋範例庫 → 取 top-3 → 組裝 prompt
...
動態 few-shot 的效果通常比靜態 few-shot 好 15-30%，因為範例跟使用者的問題更相關。缺點是需要維護一個範例庫和向量搜尋的基礎設施。
...
```
# 第一版 prompt（簡單）
你是客服分類器。根據使用者訊息，分類為 billing、technical、
