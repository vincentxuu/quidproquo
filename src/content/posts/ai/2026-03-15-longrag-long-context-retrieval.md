...
小 chunk 意味著大量候選片段。一個 10 萬字的中文文件約 15 萬 token，用 512 tokens 切割大約產生 300 個 chunk。檢索系統必須從幾百個候選中精確找到最相關的 3-5 個。
...
客服知識庫、產品 FAQ、政策手冊 ── 這些場景中，給使用者一個完整、連貫的回答比精確引用某個段落更重要。LongRAG 的大上下文讓 LLM 能生成更流暢、更完整的回答。
