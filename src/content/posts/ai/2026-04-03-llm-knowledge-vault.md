...
lang: zh-TW
tldr: "Andrej Karpathy 提出用 LLM 編譯個人知識 wiki 的框架——收集原始資料、LLM 編譯成 .md wiki、對 wiki 做 Q&A、輸出歸檔回 wiki。本文比較三種實踐路線：Karpathy 的知識庫模式、社群的經驗庫模式、以及 quidproquo 的部落格模式。"
...
| 面向 | Karpathy（知識庫模式） | 社群延伸（經驗庫模式） | quidproquo.cc（部落格模式） |
|------|----------------------|-------------------|--------------------------|
...
- **社群經驗庫模式**靠 hooks + bridge + 微調，整合成本高，各元件之間缺乏統一介面
