---
title: "LLM 知識庫實踐：從 Karpathy 的 Wiki 編譯到持續進化的 AI 工作流"
date: 2026-04-03
category: ai
tags: [llm-knowledge-base, obsidian, knowledge-management, fine-tuning, rag, claude-code, karpathy]
lang: zh-TW
tldr: "Andrej Karpathy 提出用 LLM 編譯個人知識 wiki 的框架——收集原始資料、LLM 編譯成 .md wiki、對 wiki 做 Q&A、輸出歸檔回 wiki。本文比較三種實踐路線：Karpathy 的研究者模式、社群的持續進化模式、以及 quidproquo 的出版者模式。"
description: "深入分析 Andrej Karpathy 的 LLM Knowledge Bases 框架，對照社群延伸實踐（session continuity、語音轉錄、跨 AI 同步、微調、知識衰減）與 quidproquo.cc 的出版者架構，探討 LLM 驅動知識管理的產品化方向。"
draft: false
---
...
| 面向 | Karpathy（研究者模式） | 社群延伸（進化模式） |
|------|----------------------|-------------------|
| 知識存在哪 | 本地 `.md` wiki | Obsidian vault + AI 記憶 |
| 資料來源 | 文章、論文、repo、圖片 | 同上 + 語音轉錄 |
| 時間維度 | 無衰減 | Confidence decay |
| Session 連續性 | 不需要（知識在檔案裡） | PreCompact/PostCompact hooks |
| 錯誤學習 | 無自動機制 | 錯誤計數器，3 次升級為規則 |
| 跨工具 | 單一 LLM | Bridge 模組同步多工具 |
| 微調 | 未來想做 | 已跑 16 代 |
| 輸出格式 | Markdown、slides、圖表、web UI | 未強調 |
| 人的角色 | 消費者——不碰 wiki | 策展者——設定規則讓系統執行 |

Karpathy 定義了「LLM 編譯知識」的基本模式；社群在此基礎上加入了時間維度、跨 session 維度、跨工具維度和自我修正維度。

## quidproquo.cc：出版者模式

quidproquo.cc 走的是第三條路線——把同樣的 LLM 編譯流程，轉變成公開發布的知識系統。

| 面向 | Karpathy | quidproquo.cc |
|------|---------|---------------|
| 核心理念 | LLM 編譯私人 wiki | LLM 編譯公開部落格 |
| 儲存 | 本地 `.md` 目錄 | Astro `.md` + Cloudflare D1 |
| 編譯流程 | LLM 增量編譯 wiki | `/post` skill 把對話轉成結構化文章 |
| 索引 | LLM 自維護 index，不需 RAG | Cloudflare Vectorize 語義索引 |
| 前端 | Obsidian | Astro SSG 公開網站 |
| 外部資料 | Obsidian Web Clipper | Browser Rendering API → 分塊 → D1 |
| 品質管理 | LLM health checks | OpenSpec workflow 生命週期 |
| 開放程度 | 私人 | 公開 |
| 基礎設施 | 本地檔案系統 | Cloudflare Workers + D1 + Vectorize + KV |

核心哲學一致：**知識是 LLM 的領域，人負責提問和策展。** 差別在於 Karpathy 的 wiki 服務於個人研究，quidproquo 把產出變成公開的、有索引的、有基礎設施支撐的知識系統。
...
- **quidproquo 出版模式**有產品化架構（schema migrations、rate limiting、audit logging），但定位在部落格而非通用知識庫
