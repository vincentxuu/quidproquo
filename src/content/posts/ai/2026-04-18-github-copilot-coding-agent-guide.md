...
lang: zh-TW
tldr: "GitHub Copilot Coding Agent 讓你把 Issue 指派給 Copilot，它在雲端沙箱裡自動開 branch、寫程式、跑 CI、開 PR。成功關鍵是設好 AGENTS.md，沒設定的話 agent 容易跑偏。適合定義清楚的中型任務，Business/Enterprise 方案依 premium request 計費。"
...
效能優化流程——讓 agent 先 benchmark、修改、再跑 benchmark 對比，有用戶在特定函式上拿到 99% 的效能提升。

重構——整理重複邏輯、統一命名規範、把 helper function 收進 common module。成功率跟 codebase 有 test coverage 正相關。

文件驗證——把使用者文件當成可執行指令，讓 agent 模擬 first-time user 跟著步驟走，找出哪些步驟有問題。
...
Copilot Coding Agent 包含在 **GitHub Copilot Business** 和 **Enterprise** 方案：

- Business：每個 seat 每月 300 premium requests
- Enterprise：額度更高，超用另計（約 $0.04/request）
- Copilot Pro+（個人）：每月 300 premium requests
- Copilot Free：**不包含** Coding Agent

一個 agent session 消耗 10–50 個 premium requests，視任務複雜度而定。跟 Claude Managed Agents 的 session-hour 計費不同，這套以 request 數計費，難以精確預估單一任務花費。
