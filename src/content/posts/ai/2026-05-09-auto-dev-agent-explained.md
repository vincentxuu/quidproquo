...
## 不只 Notion——可換成任何有 API 的 PM 工具

文章一直用 Notion 當例子，但整套系統 task source 是抽象的。**只有「同步 PM 工具到 GitHub issue」這一段跟 Notion 綁**，後段的 plan / code / PR / 安全閘門完全不認你用什麼工具。

實際可替換成：

| 工具 | 替換難度 |
|---|---|
| Linear | 簡單（API 乾淨、工程師圈最受歡迎）|
| Asana | 簡單（REST 簡潔，適合非工程團隊）|
| ClickUp | 簡單（API 設計類似 Notion）|
| Jira | 中等（schema 複雜但完整）|
| Trello | 中等（board / list / card 結構稍嚴格）|
| GitHub Projects v2 | 最簡單（直接 GraphQL，跟 GitHub 同源）|

要換工具的話，工程師只要改一個 module（從「Notion sync」變成「Linear sync」），其他 14 個檔案、9 份文件、PR 巡邏邏輯——**完全不用動**。

選擇邏輯通常是：「PM / 設計師習慣用什麼，就接什麼。系統去配合人，不要人配合系統」。daodao 用 Notion 是因為設計團隊一直在 Notion 上協作；如果你的團隊用 Linear 或 Jira，照樣能跑。
