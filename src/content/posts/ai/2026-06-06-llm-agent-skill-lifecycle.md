...
Benchmark 是 [SkillsBench](https://arxiv.org/abs/2602.12670)（arXiv:2602.12670）的 51 個基於 Docker 的真實世界評估任務，涵蓋 Science & Engineering、Data Analysis、Document Processing、Ops & Planning 四個領域。Backbone 全部使用 GPT-5.5。
...
- **Skill-Builder**：把 Proposer 的建議具現化為結構化 skill 資料夾（包含 SKILL.md、trigger metadata 與輔助腳本）
...
建立大規模 benchmark：17,810 個公開 agent skill、63,000+ 評估樣本，按 skill 類型分層（Data & ML、Information Retrieval 等）。測試結論很直接：MTEB 排行榜頂尖的通用 retriever 在 skill retrieval 上表現意外差勁。針對性 fine-tune 後，NDCG@10 從 66.6 提升至 83.5，提升幅度高達 16.9 points。
...
- **Skill conflict**：多個 skill 同時適用時如何協調？EvoSkill 明確把這列為未來研究方向（future work）
...
- [把 LLM Agent 的 skills / tools / code interpreter 真正組裝起來：一份論文導讀地圖](/posts/ai/2026-05-24-llm-agent-skills-tools-code-interpreter-papers/)
