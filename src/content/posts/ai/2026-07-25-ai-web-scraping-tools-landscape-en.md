...
[MarkItDown](https://github.com/microsoft/markitdown) (175k stars, MIT) by Microsoft supports the widest range of formats — PDF, Word, Excel, PowerPoint, HTML, and images, all to Markdown. It's the highest-starred project in this entire space.

**Note that this section mixes two different things.** MarkItDown is pure conversion — it reads structure that already exists in the file and maps it to Markdown, no model or GPU required. [MinerU](https://github.com/opendatalab/MinerU) (78k stars), [Marker](https://github.com/datalab-to/marker) (39k stars, Apache-2.0), and [Docling](https://github.com/docling-project/docling) (65k stars, MIT) **are parsing tools that perform OCR**: Marker runs surya underneath (Datalab's own OCR model), and Docling ships OCR backends plus a newer VlmPipeline. They handle scanned documents and complex layouts where even the text has to be inferred by a model. The performance gap is therefore an order-of-magnitude thing — in the same benchmark anydoc medians at 4.7ms while Docling takes 513.6ms, a 109x difference.

As for the division of labor: MinerU is strongest on tables and math formulas, the go-to for academic PDFs; Marker is fast and emits LaTeX; Docling emphasizes structured JSON schema and is the only cleanly MIT-licensed one of the three (Marker's model weights carry a separate OpenRAIL-M license, and MinerU requires a separate agreement above a revenue threshold). For the full selection logic and measurements on this layer, see [the three-layer document parsing ladder](/posts/ai/2026-08-06-document-parsing-three-layers-en).
