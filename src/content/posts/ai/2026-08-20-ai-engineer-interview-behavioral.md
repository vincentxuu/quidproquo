...
**Situation**：不要花太多時間描述背景。用兩句話設定場景，重點是讓面試官理解「為什麼這件事有挑戰性」。好的開頭：「我們的推薦模型在 A/B test 中整體 CTR 提升了 3%，但上線後發現對新使用者的推薦品質明顯下降。」壞的開頭：「我在上一家公司的推薦團隊工作，團隊有五個人，我們用的是 TensorFlow...」
...
**Result**：必須量化。「模型效果提升了」不夠，「新使用者的 day-7 retention 從 12% 提升到 18%，整體 CTR 維持不變」才夠。如果你的專案沒有明確的數字，至少說出規模（影響多少使用者、節省多少時間、減少多少成本）。
...
回答框架：了解基本的隱私保護技術（differential privacy、federated learning、data anonymization），但更重要的是展現你的判斷力——什麼資料不應該用來訓練、什麼時候應該讓使用者 opt-in 而不是 opt-out、GDPR/CCPA 對模型開發的實際影響。
...
「請描述一次你發現你的模型對某個使用者群體表現特別差的經歷。你做了什麼？」
