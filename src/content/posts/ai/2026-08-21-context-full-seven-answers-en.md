...
Cursor [trained self-summarization into Composer](https://cursor.com/blog/self-summarization) by folding compaction into the reinforcement learning loop, so the summaries themselves get rewarded or penalized. The gap they published is not small. The control was a heavily tuned prompt-based compaction whose summarization prompt alone ran to thousands of tokens across a dozen carefully worded sections. A trained Composer needs roughly "Please summarize the conversation," and **cut compaction error in half while using a fifth of the tokens**. SWE-grep from the previous section belongs to this route too — retrieval itself, trained into a model.

Meta's [Code World Model](https://arxiv.org/abs/2510.02387) goes further. It feeds Python execution traces in during mid-training, where **the action is a line of Python and the observation is the contents of the local variables afterward**. In their framing, this teaches the model the semantics of a program rather than only its syntax. The paper argues that trace prediction enables "grounded reasoning about code generation and execution, without requiring access to live execution environments" — no need to actually run it to know what happens. They are careful to call this an early prototype rather than something usable.

The cost is that you don't get to choose. This route is bound to a particular model and harness, not something you can toggle tonight, and switching vendors leaves it behind.
...
**Meta CWM**: 32B, dense decoder-only, interleaved sliding window attention up to a 131k context, and quantized inference on a single 80GB H100. Mid-training consumes two trajectory types: Python execution traces (stack frames recorded as JSON dictionaries of local variables) and Docker environment interactions generated by their ForagerAgent. They note that comparable agentic data elsewhere is usually collected at smaller scale during post-training, whereas CWM feeds it in at scale during mid-training. SWE-bench Verified pass@1 of 65.8% with test-time scaling, CruxEval Output 94.3%. The paper repeatedly frames the world-modeling results as "first steps" and "early results."
