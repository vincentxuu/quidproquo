---
title: "CMU 11-785 Lecture 8: Training VI: Optimizers and Regularization"
date: 2026-08-22
category: ai
...
type: guide
difficulty: 進階
tldr: "Spring 2026 Lecture 8 focuses on AdaGrad, Adam, regularization, BatchNorm, Dropout, and loss selection. This guide follows the official slides and recording and adds a small self-check that does not depend on the enrolled-course grader."
description: "A bilingual guide to CMU 11-785 Spring 2026 Lecture 8: Training VI: Optimizers and Regularization."
draft: false
series:
...
> 🌏 [中文版](/posts/ai/2026-08-22-cmu-11785-08-optimizers-regularization)

This article covers CMU 11-785 Spring 2026 **Lecture 8: Training VI: Optimizers and Regularization**. Its primary evidence is the [official slide deck](https://deeplearning.cs.cmu.edu/S26/documents/slides/lec8.optimizersandregularizers.pdf) and [official YouTube recording](https://youtu.be/4bxoPcAk1t0). It reconstructs only what those materials support and does not invent classroom dialogue or unpublished remarks.

## What this lecture addresses

The lecture centers on AdaGrad, Adam, regularization, BatchNorm, Dropout, and loss selection. Keep three things separate while reading: the model or algorithm's definition, the objective it optimizes, and the actual computational flow. The first determines the allowed function family, the second states what training prefers, and the third controls memory, speed, and numerical stability.

As Lecture 8, this topic inherits the course's earlier language of representation and training while establishing components used later. Do not merely copy terminology. For each equation, label its input, output, learnable parameters, and gradient path.
...
Tonight's minimum exercise is: **hold model and seed fixed, change only the optimizer, and compare loss curves**. Work through a tiny input by hand, reproduce it in NumPy or PyTorch, and compare the results. If they differ, inspect shapes, indexing, and reductions before questioning the theory.
...
- [Lecture 8 slides](https://deeplearning.cs.cmu.edu/S26/documents/slides/lec8.optimizersandregularizers.pdf)
- [Lecture 8 official YouTube recording](https://youtu.be/4bxoPcAk1t0)
