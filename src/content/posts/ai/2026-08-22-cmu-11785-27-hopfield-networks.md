---
title: "CMU 11-785 Lecture 27：Hopfield 網路"
date: 2026-08-22
category: ai
...
type: guide
difficulty: 進階
tldr: "Spring 2026 Lecture 27 聚焦聯想記憶、能量函數、固定點與 pattern retrieval；本文依官方 slides 與錄影重建主線，並提供不依賴課內 grader 的小型自我檢查。"
description: "CMU 11-785 Spring 2026 Lecture 27 雙語導讀：Hopfield 網路。"
draft: false
series:
...
> 🌏 [English version](/posts/ai/2026-08-22-cmu-11785-27-hopfield-networks-en)

本篇對應 CMU 11-785 Spring 2026 的 **Lecture 27: Hopfield Networks**。主要證據是[官方投影片](https://deeplearning.cs.cmu.edu/S26/documents/slides/lecture_25_Hopfield_S26.pdf)與[官方 YouTube 錄影](https://youtu.be/XmyIj53Ysmk)；下文只整理兩者能支持的內容，不補寫課堂問答或未公開的講者說法。

## 這一講處理什麼

這講的中心是聯想記憶、能量函數、固定點與 pattern retrieval。讀的時候要把「模型或演算法的定義」、「它最佳化的目標」與「實際計算怎麼流動」分開記。前者說明允許哪些函數，第二項說明訓練偏好什麼結果，最後一項才決定記憶體、速度與數值穩定性。

課程把這個主題放在完整序列的第 27 講，因此它既承接前面的表示與訓練語言，也替後續模型建立共同元件。不要只抄名詞；每遇到一個公式，就標出輸入、輸出、可學參數與沿哪條路徑傳遞梯度。
...
今晚的最小練習是：**用雙極性向量存兩個 pattern，再從帶雜訊輸入更新**。先用極小輸入手算一輪，再以 NumPy 或 PyTorch 重做，最後比較兩者。若結果不同，先檢查 shape、索引與 reduction，之後才懷疑理論。
