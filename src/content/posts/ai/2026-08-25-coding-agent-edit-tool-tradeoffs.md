...
lang: zh-TW
description: "拆解 Codex、Claude Code、OpenCode、Pi、OMP 五家的檔案編輯工具設計：unified diff 為什麼常失敗、fuzzy fallback 的代價、hashline 的 hash 錨點，對照 looplane 用受限 replace_text 把 qwen3:4b 從失敗救到 5/5 的過程。"
tldr: "LLM 寫 unified diff 壞在簿記：hunk 行數算錯、上下文行幻覺。五家的答案分成兩派——把 diff 文法做簡單（Codex 拿掉行號）、或乾脆不用 diff（Claude Code/Pi/OpenCode 的 exact replace）；OMP 更進一步用 hash 錨點綁住讀取狀態。looplane 走最小干預：保留 guarded apply_patch，加一個零模糊匹配的 replace_text，qwen3:4b eval 就從穩定失敗變 5/5。"
draft: false
---
...
任何一格算錯，`git apply` 直接回 `corrupt patch`。我在 looplane 上跑本地小模型的時候，看到的就是這種失敗：模型兩次都找對了那一行該從減號改成加號，但 patch 在新增行後就結束了、或是宣告了錯誤的 hunk 行數，`git apply --check` 判成 corrupt——語意全對、簿記全錯。另一種更陰險的失敗是上下文行幻覺：模型憑印象補出周圍的程式碼，hunk 文法合法但內容不存在，apply 一樣失敗。
...
looplane 的 M2 只有 guarded `apply_patch`，真實 provider eval 裡 qwen3:4b 反覆死在同一件事上：找到對的那一行、寫出 corrupt patch。M3 我加了受限的 exact edit，證據在 `src/looplane/tools.py#replace_text`：
...
`src/looplane/prompts.py` 的 prompt 版本 `m3-exact-edit-v1` 給了一條決策規則：小改動用 `replace_text`、建檔刪檔和複雜結構改動才用 `apply_patch`。`src/looplane/tools.py#apply_patch` 本身不動，繼續用 `git apply --whitespace=error-all` 做安全拒絕。
...
- **按模型路由編輯模式**。OMP 的 `resolveEditMode` 已經示範了 per-model variant；looplane 目前是一套工具服務所有模型，若要支援更小的模型，這是下一個自然台階。
- **審查預覽升級**。approval 目前顯示 path 和 old/new 片段，可以像 OpenCode 一樣在 mutation 前算好 diff 預覽。
- **hashline 式的內容錨點**。looplane 已經有 read-version ledger，理論上可以把 hash 放進編輯呼叫本身，讓 stale edit 在 schema 層就被拒絕，而不是事後比對。但要小心：這會把寫入耦合到讀取的輸出格式，正是 M3 筆記裡判斷「不是最小答案」的原因。
