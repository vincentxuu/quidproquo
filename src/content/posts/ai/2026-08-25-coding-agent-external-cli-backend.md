...
lang: zh-TW
tldr: "每家成熟 coding agent 都有 headless 機器介面：codex 有 `exec --json` 和更完整的 app-server JSON-RPC，claude-code 有 `-p` 加 stream-json，pi/opencode/omp 各有一種 JSON 事件流。直接把這些 CLI 當 backend 是最快的路，但代價是：它們自帶 agent loop、自己的權限模型、自己的登入。rivumi 的答案是讓外來 CLI 完整擁有它的 loop，自己只守住三件事——隔離副本、patch audit、最終驗證——並且一條 runtime 永遠不偽裝成另一條。"
description: "對照 codex、claude-code、opencode、pi、omp 五家的 headless 機器介面原始碼，說明把外部 CLI 當 coding backend 的設計張力，以及 rivumi 用隔離副本與 patch audit 畫出的安全邊界。"
draft: false
---
...
rivumi 從 M5 開始就把原則寫死在 `src/rivumi/external_runner.py#ExternalCodingRunner` 的 docstring：「外來 runtime 只能在拋棄式 Git clone 裡改東西；Rivumi 獨立驗證 patch 邊界並執行最終驗證命令。」具體拆成三道牆：
...
3. **最終驗證**：每條檢查命令走 rivumi 自己的 bounded verifier，驗完再比對一次 patch，確認檢查過程沒有動到成品。

M10/M11 把這套邊界從一次性任務延伸到長駐對話：`codex_app_server.py#CodexAppServerSession` 對 `codex app-server` 開一條 JSONL 連線、一個 thread 跑很多 turn，file-change 和指令審批透過協定路由回 rivumi；Claude 側用 Agent SDK 的 `canUseTool` 做 PreToolUse 相關性。但 vendor 的 thread/session ID 全部留在 adapter 內部，能到達 renderer 和持久層的只有 rivumi 自己生成的 ID——外來 runtime 不污染原生資料模型。
