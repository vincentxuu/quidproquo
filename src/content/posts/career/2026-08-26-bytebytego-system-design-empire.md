...
但這個案例最值得注意的不是營收數字，而是**飛輪的起點選擇**。他沒有從電子報開始，而是從一本書開始。書有幾個電子報沒有的特性：它是一次性購買（降低決策門檻）、它在 Amazon 有自然流量（不需要自己導流）、它是可信的信號（「Amazon 暢銷書作者」比「Substack 寫手」有分量）。書建立了信任，信任驅動了電子報成長，電子報規模支撐了付費產品——每一步都在前一步的基礎上疊加。
