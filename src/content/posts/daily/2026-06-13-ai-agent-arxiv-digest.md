...
「對話式推薦 Agent」是很多應用在做的場景（購物助理、娛樂推薦），用戶說「我想找部適合全家週末看的電影」，Agent 要多輪追問、推薦、根據反饋調整。現有評估方式是讓另一個 LLM 打分（LLM-as-a-judge），但這樣主觀、昂貴、結果不一致，無法嚴格驗證 Agent 是否真的滿足任務的「硬性約束條件」。
...
現有推薦 Agent benchmark 依賴 LLM 評審，無法可靠驗證 Agent 是否滿足用戶的硬性條件（例如：「必須是 PG-13 評級」、「必須是 2024 年後上映的」）。當測試標準本身不可靠，你根本不知道你的 Agent 有沒有真的學會推薦。
