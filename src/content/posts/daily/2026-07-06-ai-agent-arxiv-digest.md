...
| Workflow Induction（工作流程歸納） | 讓模型從示範或使用者回饋中「學會」如何建構工作流程，而非手動設計固定流程 |
...
把多輪文獻搜尋問題化成「建 DAG 工作流程」：Agent 不只是用關鍵字搜尋，而是主動組合搜尋算子（展開引用、過濾、重排序⋯⋯），並根據使用者回饋修改整個工作流程本身；9B 小模型訓練後 Hit@5 從 58 提升到 77，執行錯誤率降到 0%。
...
現有文獻搜尋 Agent（如 Deep Research 類產品）大多用固定管線：輸入問題→搜關鍵字→過濾→回答。問題是使用者的意圖往往含糊且動態——「幫我找跟這篇論文相關、但方法不同的工作」這種需求，固定管線無法應對。另一個痛點：即使 Agent 呼叫了正確工具，執行順序錯誤（如先過濾再搜索）也會導致結果差。PaperPilot 讓工作流程本身變成可學習、可修復的結構。
...
使用者輸入一篇 anchor paper 和一個模糊問題，想找相關文獻。但：（1）使用者說不清楚想要什麼，（2）系統不知道要先做什麼（先找引用還是先用關鍵字），（3）使用者給了回饋後系統只改查詢語句但不改搜尋策略。三個問題疊加，就是現有搜尋 Agent 的最大痛點。

#### 方法

PaperPilot 把搜尋任務化成「**工作流程歸納（Workflow Induction）**」：給定 anchor paper 和使用者問題，構建一個可執行的 DAG，節點是搜尋算子（keyword search, citation expansion, filtering, scoring, reranking, evidence extraction），邊是執行依賴。使用者的多輪回饋同時更新查詢語句和 DAG 拓撲結構本身。訓練方式：SFT（監督微調，模仿正確工作流程示範）打底，再用 preference optimization（對比「被破壞的工作流程」和「正確工作流程」）提升穩健性。

#### 為什麼重要

這篇論文的貢獻不只是搜尋系統本身，而是示範了一種可泛化的訓練範式：**「讓模型學習建 DAG 工作流程，而非只學呼叫工具」**。這個思路可以遷移到任何需要「多步驟工具組合 + 使用者互動修正」的 Agent 應用。
