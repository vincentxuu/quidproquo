...
現有 agent benchmark（如 SWE-bench、Terminal-Bench）大多評測「能不能在幾分鐘內完成一個明確定義的小任務」，且只看最終成敗。真實工作中，工程師要跑的是「復現論文實驗」、「從零優化能源系統」這種需要一到兩小時、橫跨數百步驟的長程任務。現有評測只給二元 0/1 分，無法反映「agent 走了 80% 然後卡住」這種場景，benchmark 訊號過於稀疏。
