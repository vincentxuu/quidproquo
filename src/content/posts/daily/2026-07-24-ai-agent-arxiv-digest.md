...
分析 15 個主要開源 Agent 框架從 2022 年底到 2026 年初的四年數據：808,042 個 star、73,997 個 PR、86,241 個 commit、987,330 個用戶帳號。把數據分成「知名度（Awareness）」、「採用度（Adoption）」、「留存率（Retention）」三個層次來評估生態健康度。
...
略讀
如果你的 Agent 需要長期追蹤多個用戶或實體之間的關係（例如 CRM Agent、個人助理 Agent），這篇值得細看；其他情境了解概念即可。

### 領域背景

現代 LLM Agent 記住用戶歷史的常見做法是把對話存成 profile（個人檔案），用戶問問題時撈出來放進 context。但有個盲點：如果問題需要把多個人的資訊串起來（例如「我同事 Alice 的老闆喜歡什麼？」——要先找 Alice 的 profile，再找她老闆的 profile），傳統的單跳（single-hop）檢索就會失敗，因為系統不知道要自動去找「Alice 的老闆」的相關資料。
...
現有 Agent 記憶系統只能做「直接查詢」：用戶問什麼就找對應的 profile，無法自動跨越多個實體做串聯推理。而且現有 benchmark 大多只測單跳回憶，根本沒有測多跳能力，問題被長期忽略。
...
不需要額外建構知識圖譜、不需要額外 API 成本，就能讓 Agent 記憶具備多跳推理能力。對需要管理多用戶長期記憶的 Agent 平台，這是一個輕量且效果合理的方案。
...
- 如果你的 Agent 需要記住多個用戶或實體間的關係，可以借鑒 ProGraph「在 profile 文字裡掃描人名」的思路，不需要圖資料庫，成本低。
