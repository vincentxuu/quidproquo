...
學術界做 agent 研究的常用辦法是：設計精心控制的 benchmark，在乾淨環境下測試。但生產環境有各種「骯髒」現實：API 不穩定、使用者輸入格式奇怪、下游系統有權限限制、出錯了要能 fallback。這篇論文填補的正是從「benchmark 上的好成績」到「production 裡真的能跑」之間的那段距離。
