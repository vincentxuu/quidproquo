...
- **tech stack**：Zig + x64dbg plugin API + HTTP server，零外部依賴、跨平台單一二進位輸出
...
- **tech stack**：TypeScript，內建 MCP client，模型可換 Qwen3-Coder-Plus 或其他相容 endpoint
...
- **tech stack**：Rust（新版引擎）＋內嵌 Web UI，支援 curated 模型庫本地下載，自動代管 Ollama
...
- **tech stack**：TypeScript，CLI + MCP server 雙模式，整合 Gemini／OpenAI／Kie AI 三家生成 API
