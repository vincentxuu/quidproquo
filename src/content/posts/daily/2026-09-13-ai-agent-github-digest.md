...
- **技術棧**：Rust + git worktree + shell hook 系統
...
- **是什麼**：巴西團隊做的開源「AI 銷售作業系統」——自架 CRM，原生用 AI agent 在 WhatsApp 上接待、篩選、成交客戶，對標 Kommo、Octadesk、Intercom。
...
- **技術棧**：Next.js 16 + Supabase（Postgres + pgvector）+ WAHA（WhatsApp）+ Vercel AI SDK v7
...
- **技術棧**：Claude（Anthropic）+ TypeScript + LanceDB（語意記憶）+ x402 協定
...
- **技術棧**：Go + Docker sandbox + 多 LLM provider（Anthropic、OpenAI、DeepSeek 等）+ Langfuse 可觀測性
