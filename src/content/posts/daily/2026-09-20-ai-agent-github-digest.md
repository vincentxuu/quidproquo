...
- **技術棧**：JavaScript + Claude Code / Codex / Opencode / Cursor 整合層
...
- **技術棧**：Laddered Simple Attention Network（Monarch Hadamard MLP 取代 FFN + GQA attention + engram n-gram 記憶）
...
- **技術棧**：Python + tree-sitter AST parsing + Leiden 社群偵測演算法
...
- **技術棧**：C# editor addon + ReflectorNet 反射框架 + 自架或雲端（ai-game.dev）MCP server
