...
- **練習時用 AI 直接要答案，考試會比從沒用過還差。** 一場近千人的隨機對照試驗：練習正確率 +48%，AI 收走後考試 −17%。
- **多數「AI 有助學習」的研究測的是另一件事**——AI 還在手邊時的表現，不是 AI 收走之後剩下什麼。
- **可操作的規則只有一條：先自己卡一次，再問 AI。** 把 AI 放在提示的位置，不要放在答案的位置。
- **順就是訊號。** 如果過程變順了，那通常就是你沒在學的時候。

後面說明每一條的證據強度。先從一件該講而很少人講的事開始。
...
有人直接把 AI 收走再考一次，答案是：**剩下的比沒用過還少。**

土耳其近千名高中生被分成三組做數學練習——一組用接近原生 ChatGPT 的 GPT Base、一組用加了教師設計提示的 GPT Tutor、一組只有課本和筆記（[Generative AI without guardrails can harm learning](https://pmc.ncbi.nlm.nih.gov/articles/PMC12232635)，2025 年發表於 PNAS 的隨機對照試驗）：
...
- 2026-08-18：可讀性調整——開頭加入四點結論摘要，RCT 一節改以發現開頭而非研究者姓名。內容與證據未變動。
