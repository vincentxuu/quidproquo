...
**正2 / 槓桿 ETF（Leveraged ETF）**
每日追蹤標的指數報酬的 2 倍。以 00631L 為例，若台灣 50 指數當天漲 1%，00631L 目標漲 2%；跌 1% 則目標跌 2%。每日重設意味著槓桿倍數在每個交易日結束後歸位，不是固定在你買入的那天。

**每日重設（Daily Reset）**
槓桿 ETF 每天結算後重新設定槓桿倍數的機制。好處是下跌時損失有上限（不會因為每天跌 20% 連跌幾週就歸零）；代價是在震盪行情中會產生波動損耗，詳見下條。

**波動損耗（Volatility Decay）**
因每日重設導致的結構性損耗。漲 10% 再跌 10% 不是回到原點——而是 99 元。2 倍槓桿版漲 20% 再跌 20%，剩下 96 元。震盪越頻繁，這個損耗越大。在趨勢市場中，複利效果能超過損耗；在長期盤整市場中，損耗會持續侵蝕本金。

**路徑依賴（Path Dependency）**
最終報酬不只取決於起終點的價格差，還取決於中間的漲跌順序。同樣是從 100 漲到 110，一路順漲和先跌後漲的槓桿結果可能不同。這是槓桿 ETF 和直接買入並持有現貨 ETF 最大的差別之一。

**Beta / 曝險（Exposure）**
衡量組合相對於大盤波動的敏感度。Beta = 1 表示組合跟大盤等幅波動；Beta = 1.2 表示大盤漲 10%，組合目標漲 12%；大盤跌 10%，組合目標跌 12%。這套系統的目標是 Beta = 1.2，由 60% 正2（2 倍槓桿）和 40% 現金（Beta = 0）組合而成：0.6 × 2 + 0.4 × 0 = 1.2。

**再平衡（Rebalancing）**
定期或在觸發條件下把資產比例還原到目標配置的操作。這套系統用 Beta ±10% 作為觸發點，不是固定時間（月或季）。觸發條件式的再平衡可以更精確地在市場偏離時介入，而不是固定週期硬調整。

**波動收割（Volatility Harvesting）**
再平衡在波動市場中產生的額外報酬效應。每次「高賣低買」的再平衡操作，都會讓組合在下一輪漲跌中有更多低點籌碼參與上漲，長期下來報酬超過靜態持有。

**效率市場假說（Efficient Market Hypothesis, EMH）**
市場在任何時刻的價格都已充分反映所有公開資訊，因此不存在可以持續利用的「錯誤定價」。Malkiel 的版本是：公開資訊到達市場後幾乎立刻被消化，個人投資者無法系統性地從公開資訊中取得超額報酬。

**SPIVA（S&P Indices Versus Active）**
標普指數公司每年發布的報告，追蹤主動管理型基金相對於對應指數的長期績效。2023 年報告顯示，在任意 15 年以上的區間，超過 85% 的美股主動基金績效輸給對應指數。

**滾動報酬（Rolling Return）**
從不同起始日期開始、持有相同時間長度的歷史報酬分佈。例如「所有 5 年持有期的報酬」包含從每一天開始持有 5 年的結果。滾動報酬能更真實呈現「在任意時間點買入、持有固定期間」的機率分佈，比單一歷史績效更有參考價值。
