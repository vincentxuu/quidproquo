...
lang: en
tldr: "This series has cited the specification four times without ever reading it end to end. Reading it turns up three things that cut against intuition. First, of the seven mandatory requirements, only three apply to the aircraft — identity authentication, network ports, malware and vulnerability scanning all land on the ground control station, so the centre of gravity is not the drone. Second, the communications requirement can be passed without encryption at all, provided the manual or the box states the reason or the risk; the specification asks for disclosure, not encryption. Third, the five items that actually test resilience — position spoofing, position jamming, link loss, known firmware vulnerabilities, and an app security label — all sit in Chapter 8, and Chapter 8 is optional. Incidentally, one pass criterion for the spoofing test is 'the unit enters forced-landing or return mode' — exactly the behaviour I measured in a simulator in the previous post."
