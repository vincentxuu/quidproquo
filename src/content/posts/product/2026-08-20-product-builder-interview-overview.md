...
傳統 PM 的核心能力是**協調**：收集需求、寫 PRD、排優先序、推動工程和設計團隊把東西做出來。好的 PM 是翻譯機，把商業目標翻成技術需求，把用戶回饋翻成可執行的 backlog。
...
**角色邊界**。傳統 PM 有明確的分工界線——設計師畫圖、工程師寫 code、PM 管流程。Product Builder 的界線模糊得多，你可能同時在做用戶訪談、調 CSS、看 analytics dashboard。
...
1. **Product Sense**——用戶洞察、問題定義、功能優先序。你能不能從一個模糊的需求裡找到真正值得解的問題？
