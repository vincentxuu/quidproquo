...
lang: zh-TW
tldr: "遙控鏈路是資安檢測規範點名的三個模組之一，但很少有中文資料說它到底怎麼運作。ExpressLRS 是開源的，所以可以直接讀：跳頻順序由一句綁定口令經 MD5 推出 UID，再餵給一個 a=214013、c=2531011 的線性同餘產生器算出來——完全可複現，我把它移植成 Python 並跟原始 C 碼對過，七種頻道數全部逐位元相同。鏈路本身沒有任何加密，只有 14 bit CRC。而台灣 LP0002 的規定很妙：2.4 GHz 跳頻系統用滿 75 個頻道以上可以發到 1 W，不足 75 個只能發 0.125 W——ExpressLRS 用 80 個。頻道數不只是抗干擾參數，它決定了你合法能發多大功率。另外那條名字就叫「航空模型飛機遙控器」的條文，只給 27／72 MHz 而且限單向控制，今天的雙向遙控器根本不走那條。"
