...
lang: en
tldr: "The RC-link post ended by saying it hadn't touched the video downlink and that the topic deserved its own piece. This is that piece. Digging in turned up two independent problems. The first is frequency: LP0002 opens 5725–5850 MHz for 5.8 GHz — only 125 MHz — while the standard channel table on consumer FPV video transmitters spans 5645–5945 MHz, a full 300 MHz. Of the forty classic channels, only twenty-four have carriers inside the window; count the roughly 20 MHz emission width and only eighteen fit, and Band E has none at all. The second problem is harder. §4.10.1.1 says 2.4 GHz takes 'transmitters using frequency hopping or digital modulation'; for 5.8 GHz it says only 'transmitters using frequency hopping'. Analogue FPV video is a fixed-frequency analogue FM transmission — it does not hop, and one channel is about 20 MHz wide, while the rule demands at least 75 hopping channels each with a 20 dB bandwidth no greater than 1 MHz. It isn't that analogue FPV violates a clause. It's that no clause in this document is shaped like it."
