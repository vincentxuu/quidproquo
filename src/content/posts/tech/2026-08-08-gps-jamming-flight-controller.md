---
title: "GPS 被干擾的那七秒：飛控怎麼發現、怎麼決定，以及為什麼「偵測干擾」預設是關的"
date: 2026-08-08
type: deep-dive
category: tech
tags: [drone, gnss, ekf, px4, ardupilot]
lang: zh-TW
tldr: "資安檢測規範點名三個模組，飛控和通訊前兩篇拆完了，這篇拆第三個：衛星定位。我在 SITL 裡把無人機飛到 28 公尺，然後打開模擬器內建的 GPS 干擾——模擬時間約 7 秒後，飛控自己宣告 EKF 失效、切成 LAND、降落、上鎖。它沒有飛走，也沒有墜毀。而讀 PX4 的原始碼發現一件反直覺的事：EKF2_GPS_CHECK 有 12 道閘門，第 9 位是「詐騙」、第 11 位是「干擾」，但預設值 2047 只開到第 10 位——干擾偵測預設是關的。理由其實合理：估計器自己就抓得到干擾（變異數會爆），詐騙卻只能靠接收機告訴它。"
