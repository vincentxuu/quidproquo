...
lang: zh-TW
tldr: "把 Skybrush 那套開源燈光秀韌體（ArduPilot 的 CollMot 分支）整個拉下來讀，9,199 行裡搜遍 neighbour、peer、collision、avoidance、swarm——沒有任何一架飛機知道別架飛機的存在。每架機在自己的 SD 卡上放一個 show.skyb，用 GPS 時間對時，10 Hz 播放自己那條軌跡。兩百架之間同步的東西，全部內容是一個 GPS 週內秒加一個毫秒偏移。唯一長得像防撞的「泡泡圍籬」量的是自己離自己的預定位置多遠（預設 10 公尺），而預設動作是 1 = 只回報。防撞是在地面的編排軟體裡算完的。台灣這邊：遙控無人機資安檢測規範第 7 章是群飛專章，十二個檢測項目，「遙控無人機」那一欄從頭到尾都是「-」——測的是交換器、路由器、防火牆、無線寬頻分享器和網頁管理介面，引用標準是《物聯網場域資安防護評估指引》和《無線寬頻分享器資安測試規範》。它不是在測群飛，是在測辦一場群飛所需要的那個機房。而立法理由寫的是「遭受惡意之資通訊干擾，恐令遙控無人機失控墜毀」——那條風險鏈的單點是 GNSS，而 GNSS 欺騙與干擾在規範裡是第 8 章選測，不在群飛的合規條件裡。這個形狀有跡可循：2022 年 11 月 11 日的行政院會議紀錄明文要求「納入群飛的任務軟體檢測」，四年後的 V2.0 第 7 章一項任務軟體都沒有。"
