---
title: "Looplane 的 state-first event journaling：為什麼 manifest commit 跟 JSONL append 之間的 crash 要靠 state 修"
date: 2026-08-30
category: tech
type: deep-dive
tags: [looplane, coding-agent, crash-recovery, journaling, file-locking]
lang: zh-TW
tldr: "Looplane 同時維護 append-only 的 `events.jsonl` 與原子替換的 `session.json`，用 sequence reconciliation 判斷 crash 後能否安全續跑。這份 event contract 現在也支援 deterministic replay、JSON replay、從指定 sequence 建立 fork seed，以及在不重放副作用的前提下建立新 workspace；遇到停在 `tool.started` 或 `verification.started` 的不確定狀態仍然 hard fail。"
description: "深入 Looplane 的 crash-safe 設計：atomic_write_json 用同目錄 temp file + fsync,EventWriter 用 O_APPEND + fsync,SessionWriterLease 用 fcntl.flock + O_NOFOLLOW,claim_and_validate_resume 的 manifest_was_ahead 邏輯,以及為什麼 tool.started / verification.started 不能自動 resume。"
series:
  name: "Looplane 架構拆解"
  order: 12
draft: false
...
`events.jsonl` 的寫入在 `src/looplane/events.py:46-57`:
...
兩個寫入路徑合在一起,Looplane 才能同時說兩句話:**「events.jsonl 紀錄的每一行都是真實發生過的」** 跟 **「session.json 永遠是某個一致狀態的快照」**。前者是審計,後者是控制;它們各自用最適合的檔案語意。
...
`session.py:194-212` 的 `acquire_writer` 是 Looplane 怎麼防止「兩個 process 同時寫同一個 session」的:
...
關鍵點是 **`fcntl.flock` 是 advisory lock**,只在同一個檔案描述符上有效——但因為 `SessionWriterLease` 把 descriptor 包進物件且整個 run 不關(`lease.release` 只在 finally 跑),同一個 process 的後續 save 一定看到自己的 token。同時另一個 process 嘗試 acquire 同一個 `.writer.lock` 會拿到 `BlockingIOError` → `SessionBusyError`,所以「兩個 looplane run 同時搶同一個 session」在 OS 層就被擋下來。
...
這也是最新 TUI / session 工具能成立的原因:`/resume` 不是從螢幕文字猜狀態,`/history` 不是倒放 vendor transcript,`looplane sessions` 也不是掃一堆臨時 log。它們共同讀的是 versioned `session.json` 與 append-only events。外部 CLI runtime 的 vendor session id 不必變成 Looplane 的 source of truth;Looplane 只保存自己能驗證的狀態、patch、usage 與 event sequence。

這條 lifecycle 到 fork 為止，不延伸到 SDK facade 或 WebSocket conversation ownership。[下一篇](/posts/tech/2026-08-30-looplane-context-compaction)處理 context pressure、compaction 與 reinjection：當 manifest 裡的 conversation 長到接近上限時，如何保住決策與 workspace 狀態。SDK 與 embedding boundary 留到 order 17。
...
- [Looplane `session_replay.py`（2ed5efb）](https://github.com/vincentxuu/looplane/blob/2ed5efb94cb1f344f8b360256fd6b4aae60fe34c/src/looplane/session_replay.py)——deterministic replay reducer 與 fork seed
- [Looplane M3 文件](https://github.com/vincentxuu/looplane/blob/main/docs/stages/m3-reliable-editing-real-provider-eval.md)——checkpoint / resume 的官方里程碑紀錄
- [Linux `flock(2)` advisory locks](https://man7.org/linux/man-pages/man2/flock.2.html)——Looplane writer lease 使用的系統呼叫語意
...
- [Crash-only software: designing for recovery](https://lwn.net/Articles/191059/)——「crash 是常態不是例外」的工程典範,Looplane 的設計哲學對齊
- [Pydantic v2 — frozen models 與 extra=forbid](https://docs.pydantic.dev/latest/concepts/strict_mode/)——`SessionManifest` 的不可變保證從哪來
- [filesystem `fsync(2)` 與 durability](https://man7.org/linux/man-pages/man2/fsync.2.html)——Looplane 在 `durable=True` 時多處顯式 fsync 的系統呼叫文件
