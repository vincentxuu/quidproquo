---
title: "Looplane's tool isolation: path allowlists, strict argv, process groups, and credential-free subprocesses"
date: 2026-08-30
category: tech
type: deep-dive
tags: [looplane, coding-agent, ai-agent, python, sandbox]
lang: en
tldr: "This article follows one Looplane tool call through its mechanical execution boundary: `SafePathPolicy` for paths and symlink escape, fixed argv with `shell=False`, a sanitized subprocess environment, read-version hashes plus atomic replace for writes, and process-group cleanup at timeout. Permission policy, OS containment, and tool programs are reserved for later articles."
description: "A deep dive into Looplane's tool isolation design: SafePathPolicy path allowlists, strict VerificationCommand argv, credential-free sanitized_subprocess_env, process-group timeouts, _BoundedCapture output caps, and why prompt injection has little leverage at this layer."
series:
...
> 🌏 [中文版](/posts/tech/2026-08-23-looplane-tool-isolation)

The previous article separated the native loop from the external CLI runtime. Either lane eventually reaches the same question: how does a path, argv, or write request land safely in a workspace? This article follows only Looplane's mechanical execution boundary: path, argv, environment, atomic writes, and timeout cleanup. Whether a call should be allowed, asked, or denied belongs to order 9; OS containment belongs to order 10; programs and transactions belong to order 11.
...
Because `Path.resolve(strict=False)` follows symlinks, `workspace/link -> /etc/passwd` cannot pass a naive prefix check. Looplane checks the resolved path against the resolved workspace root. `_walk_files` also uses `followlinks=False`, so directory walking cannot be pulled outside the workspace by a symlink.
...
For timeouts, Looplane uses `start_new_session=True` and signals the process group:
...
The replacement itself writes to a temporary `target.name.looplane-replace-<uuid>` file, fsyncs it, atomically replaces the target with `os.replace`, then fsyncs the parent directory. It opens with `O_NOFOLLOW` and `O_EXCL`, and rolls back to original bytes if a later step fails.
...
For the seven single-step base tools described here, Looplane's "the loop belongs to the harness" claim becomes concrete: **each side effect is statically narrowed by Python types and dynamically checked at runtime**. Dynamic MCP capabilities and other subprocess-owning integrations have separate boundaries. The native model sees a filesystem subset, an argv-name allowlist, and bounded credential-free subprocesses.

The external CLI runtime does not remove this principle; it moves the boundary later. Claude Code, Codex CLI, OpenCode, Pi, and OMP may use their own tools inside their own loops, but only inside a disposable clone. When they return to Looplane, their patch still passes path-bounded audit, binary/symlink/rename/copy checks, final verification, and source-invariant comparison. The Cloudflare slice follows the same shape: Worker ingress validates paths and argv, the Sandbox runs the native bounded toolset, and provider credentials remain in the Worker model proxy.

If this layer holds, prompt injection has less host surface to exploit. It still does not answer who has authority to execute a call or what kernel resources a process can reach. The next layers are [allow/ask/deny permissions](/posts/tech/2026-08-30-looplane-permission-layering-en), [local OS sandboxing](/posts/tech/2026-08-30-looplane-local-os-sandbox-en), and [tool programs with rollback-capable transactions](/posts/tech/2026-08-30-looplane-tool-program-transactions-en). None can be replaced by the path and argv checks described here.

## References

- [Looplane official repo](https://github.com/vincentxuu/looplane) -- the ground truth for all code references in this article
- [Looplane `tools.py` at 2ed5efb](https://github.com/vincentxuu/looplane/blob/2ed5efb94cb1f344f8b360256fd6b4aae60fe34c/src/looplane/tools.py) -- core tools, path-bounded search, atomic writes, and timeout dispatch
- [Looplane M1 docs](https://github.com/vincentxuu/looplane/blob/main/docs/stages/m1-local-harness.md) -- harness design notes
