---
title: "A map of Looplane: how one coding-agent task crosses workspaces, runtimes, tools, and events"
date: 2026-08-30
category: tech
type: deep-dive
tags: [looplane, coding-agent, ai-agent, python, software-architecture]
lang: en
tldr: "Looplane turns a coding-agent task into inspectable boundaries: native side effects cross Looplane tools, permissions, and sandboxing, while external runtimes retain their own loops and tools before returning a patch for Looplane audit. This article maps the planned 20-part series."
description: "A map of Looplane from task ingress to Cloudflare remote execution, covering workspace, prompt, runtime, tool, event, and integration boundaries and the reading order for this 20-part series."
series:
...
disposable workspace ---- instructions / project context / explicit memory
      ...
      +------------------ prompt ------------+
          ...
          native AgentRunner      external CLI runtime
                 |                       |
       Looplane tools / permission    CLI-owned tools / auth
           / OS sandbox                   |
                 |                       |
...
The diagram deliberately keeps the two lanes separate. Native tool calls cross Looplane's path, argv, permission, and OS-sandbox boundaries. External CLIs retain their own loops, tools, and authentication; Looplane does not pretend to own those side effects. The lanes meet again at returned-patch audit, verification, and artifacts. A prompt that merely says “be careful” is not one of these boundaries.
...
|---:|---|---|
| 0–7 | How does a task enter Looplane and select an execution path? | TUI/CLI, workspace, prompt, native loop, provider adapters, routing economics, external runtime |
...
Looplane has two distinct execution paths. In the native lane, `AgentRunner` owns the model/tool loop. The model returns structured output and tool calls; Looplane owns execution, verification, repetition detection, and termination. In the external lane, official CLIs such as Codex or Claude Code remain complete runtimes. Looplane owns the surrounding workspace, capability handoff, returned-patch audit, and shared artifacts.
...
- For capability extension, read [MCP](/posts/tech/2026-08-30-looplane-native-mcp-authorization-en), [skills/hooks/plugins](/posts/tech/2026-08-30-looplane-skills-hooks-plugins-en), and [subagents](/posts/tech/2026-08-30-looplane-subagent-scheduling-en) in that order.
- For product integration, finish with [SDK/WebSocket](/posts/tech/2026-08-30-looplane-sdk-conversation-websocket-en), [IDE/LSP](/posts/tech/2026-08-30-looplane-ide-lsp-vscode-bridge-en), and [Cloudflare](/posts/tech/2026-08-23-looplane-cloudflare-deployment-en).
...
- [Looplane GitHub repository](https://github.com/vincentxuu/looplane)
