---
title: "Looplane 的 Subagent Scheduling 與 Parent-owned Transactions"
date: 2026-08-30
category: tech
type: deep-dive
tags: [looplane, coding-agent, subagents, scheduling, transactions]
lang: zh-TW
tldr: "Looplane 把每次 subagent dispatch 正規化成最多四個節點的 dependency waves，讓同 wave 的唯讀 child 在隔離 workspace 平行分析，再由 parent 重新走 hook、approval 與 transaction 執行修改。"
description: "追蹤 Looplane subagent dependency graph、wave concurrency、isolated child workspace、bounded handoff，以及 parent-owned transaction 的授權邊界。"
series:
  name: "Looplane 架構拆解"
  order: 16
draft: false
...
> 🌏 [English version](/posts/tech/2026-08-30-looplane-subagent-scheduling-en)

[上一篇](/posts/tech/2026-08-30-looplane-skills-hooks-plugins)拆開 repository-local 擴充的三種權限。當工作可以拆成幾條獨立調查線時，[Looplane](https://github.com/vincentxuu/looplane)還能 dispatch subagents；這裡的核心不是「多開幾個 agent」，而是誰擁有 workspace 與修改權。
...
因此 `dispatch_subagents` 本身分類為 read。真正碰到修改與執行的時刻，是 parent transaction；那時仍套用[order 11](/posts/tech/2026-08-30-looplane-tool-program-transactions)說明的 transaction 與 rollback 契約。Reviewer 說「可以」不會取代 operator approval，child workspace 也不會直接 merge 回 parent。
...
[下一篇](/posts/tech/2026-08-30-looplane-sdk-conversation-websocket)進入 embedding boundary，再由 IDE bridge 接到 order 19 的 Cloudflare capstone。
