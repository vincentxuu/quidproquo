---
title: "Rivumi Subagent Scheduling and Parent-owned Transactions"
date: 2026-08-30
category: tech
type: deep-dive
tags: [rivumi, coding-agent, subagents, scheduling, transactions]
lang: en
tldr: "Rivumi normalizes each subagent dispatch into dependency waves of at most four nodes, runs read-only children concurrently in isolated workspaces, then makes the parent repeat hooks, approval, and transaction execution for any modification."
description: "Trace Rivumi's subagent dependency graph, wave concurrency, isolated child workspaces, bounded handoffs, and the authority boundary of parent-owned transactions."
series:
...
> 🌏 [中文版](/posts/tech/2026-08-30-rivumi-subagent-scheduling)

The [previous article](/posts/tech/2026-08-30-rivumi-skills-hooks-plugins-en) separated three kinds of repository-local extension authority. When a task has independent investigation tracks, [Rivumi](https://github.com/vincentxuu/rivumi) can also dispatch subagents. The central question is not how many agents run, but who owns the workspace and modification authority.
...
`dispatch_subagents` is therefore classified as read. Modification and execution appear only at the parent transaction stage, where the [order 11 transaction contract](/posts/tech/2026-08-30-rivumi-tool-program-transactions-en) still applies. Reviewer acceptance cannot replace operator approval, and a child workspace is not merged directly into the parent.
...
The [next article](/posts/tech/2026-08-30-rivumi-sdk-conversation-websocket-en) enters the embedding boundary, followed by the IDE bridge and the order 19 Cloudflare capstone.
