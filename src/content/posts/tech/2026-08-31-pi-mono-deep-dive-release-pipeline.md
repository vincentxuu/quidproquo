---
title: "pi-mono 深度導讀 16：Release 流程、Lockstep Versioning、Binary Build、Trusted Publishing——從代碼到 npm 發佈的完自動化管線"
date: 2026-08-31
type: project
category: tech
...
series:
  name: "pi-mono 深度導讀"
  order: 16
tldr: "Release 完整流程：Lockstep Versioning（所有套件同版本）、CHANGELOG 更新、Local Smoke Test 驗證、Release Script 自動化、Binary Build (Bun + Node)、npm-shrinkwrap.json 鎖死傳遞依賴、GitHub Actions OIDC Trusted Publishing、R2 Release Marker 驗證、pi.dev/api/latest-version 公告、Announcement Verification 確保發佈成功。"
description: "深入 pi-mono 發佈管線：Lockstep Versioning 策略、版本號語義、CHANGELOG 管理、Local Smoke Test 雙平台驗證、Release Script 自動化步驟、Bun/Node Binary Build 細節、npm-shrinkwrap.json 生成、GitHub Actions OIDC Trusted Publishing 免 Token 發佈、R2 Release Marker 驗證機制、pi.dev 版本公告 API、Announcement Verification 確保 npm 可用。適合研究 TypeScript Monorepo 發佈自動化、供應鏈安全、Binary 分發的工程師。"
draft: false
---

> 🌏 [English version](/en/posts/tech/2026-08-31-pi-mono-deep-dive-release-pipeline-en)
