...
Two weeks into using Claude Code, you notice strange files appearing in your project root and home directory: `.claude/settings.local.json`, `~/.claude.json`, a pile of markdown under `~/.claude/projects/` that you do not remember writing. This post maps both layers file by file — who creates each one, whether it should be committed, and what overrides what. It is the most useful follow-up to [the agentic loop post](/posts/tech/deep-dive/2026-08-26-claude-code-how-it-works-en), because every extension mechanism covered later in this series drops its files somewhere on this map.
...
**Settings merge; CLAUDE.md concatenates.** List-type settings keys (like `permissions.allow`) combine across all scopes as a union; scalar keys (like `model`) are won by the most specific scope. CLAUDE.md files are all loaded into context together — not inherited-and-overridden. Global and project content coexist; conflicts are resolved by how you word the instructions.
...
| File / folder | Purpose |
|---|---|
| `CLAUDE.md` | Personal preferences, loaded alongside the project's CLAUDE.md; project takes priority on conflict |
...
`~/.claude/projects/<project>/memory/` is the only zone you never write — Claude maintains it while working: build commands, architecture notes, debugging insights. `MEMORY.md` is the index; the first 200 lines or 25KB load at each session start. Longer topics get split into topic files (`debugging.md`, `architecture.md`) that are read back only when a related task comes up. On by default; toggle with `/memory`. These are plain markdown — edit or delete freely, though Claude will keep updating them afterward.
...
- [How Claude Code works — Claude Code Docs](https://code.claude.com/docs/en/how-claude-code-works) — auto memory and CLAUDE.md loading behavior at session start
