...
import type { PipelineCallbacks } from './graph'
...
export async function runPipeline(
  ...
  if (engine === 'langgraph') return runLangGraphPipeline(input, callbacks)
...
async function runManualPipeline(
  input: Parameters<typeof runPipeline>[0],
  callbacks: PipelineCallbacks,
  engine: RagRuntimeConfig['pipelineEngine']
): Promise<GraphState> {
