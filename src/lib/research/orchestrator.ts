...
import { initialState, type GraphState, type RagRuntimeConfig, type SearchResult, type RagMessage } from '../rag/state'
import { plannerNode } from '../rag/agents/planner'
import { researchNode } from './nodes/research'
import { writerNode } from '../rag/agents/writer'
import { criticNode } from '../rag/agents/critic'
import { resolveProviderApiKeys } from '../rag/provider-key-store'
import { loadRagSettings, withConfigOverrides } from '../rag/settings'
import { SUPPORTED_PROVIDERS } from '../rag/providers'
...
export async function runDeepResearch(params: {
  brief: string
  config: DeepResearchBody['config']
  env: Env
  origin: string
}): Promise<DeepResearchResult> {
  const { brief, env: runtimeEnv, origin } = params
  ...
  if (!storage.kv && !storage.db) {
    throw new Error(`Deep research storage is not configured for mode: ${storageMode}`)
  }
  ...
  if (!cacheSaved && !dbSaved) {
    throw new Error('Deep research result persistence failed')
  }
