...
export function selectPageChunks<T extends ChunkRow>(rows: T[], query: string, limit = PAGE_CHUNK_LIMIT): T[] {
...
export async function fetchPageChunks(input: { slug: string; query: string; limit?: number }): Promise<SearchResult[]> {
  ...
  const { results } = await DB.prepare(
    ...
    LIMIT 200`
  ).bind(input.slug).all<ChunkRow & { slug: string; title: string; date: string }>()

  return selectPageChunks(results ?? [], input.query, input.limit).map(row => {
