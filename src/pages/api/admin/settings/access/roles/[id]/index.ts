...
export const POST: APIRoute = async ({ params, request, cookies, locals }) => {
  ...
  if (!existing) {
    if (!body.isJson) {
      return redirectWithNotice(
        body.redirectTo || '/admin/settings/access?tab=roles',
        'error',
        '找不到角色。',
      )
    }
  ...
  try {
    const permissionResponse = await requireRbacMutationPermission({
      ...
      redirectTo: body.redirectTo || `/admin/settings/access/roles/${roleId}`,
      isJson: body.isJson,
    })
    ...
    if (body.action === 'delete') {
      if (RESERVED_ROLES.has(existing.name)) {
        if (!body.isJson) {
          return redirectWithNotice(
            body.redirectTo || `/admin/settings/access/roles/${roleId}`,
            'error',
            '內建角色不可刪除。',
          )
        }
