...
export const POST: APIRoute = async ({ params, request, cookies, locals }) => {
  ...
  if (!existing) {
    if (!body.isJson) {
      return redirectWithNotice(
        body.redirectTo || '/admin/settings/access?tab=users',
        'error',
        '找不到使用者。',
      )
    }
  ...
  try {
    const permissionResponse = await requireRbacMutationPermission({
      ...
      redirectTo: body.redirectTo || `/admin/settings/access/users/${userId}`,
      isJson: body.isJson,
    })
    ...
    if (!body.email || !body.email.includes('@')) {
      if (!body.isJson) {
        return redirectWithNotice(
          body.redirectTo || `/admin/settings/access/users/${userId}`,
          'error',
          '請輸入有效的電子郵件。',
        )
      }
